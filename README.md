...
# ВАЖНО
  ...
  * это серверная часть - `мониторинг` есть проверка работоспособности компонентов системы - баз данных и LLM
  * `мониторинг` качества ответов LLM осуществляется в клиентской части приложения
...
Убедитесь, что следующие файлы находятся в корневой директории проекта `TUSUR_Project`:
...
Этот процесс завершает контейнеризацию и развертывание приложения. Теперь можно использовать его в любой среде, где установлен Docker!
